...
@sv.on_fullmatch("谁艾特我", "谁at我", "谁AT我", "谁艾特我？", "谁At我", "谁@我", '谁@我了',"wam")
...
async def message_monitor(bot: HoshinoBot, ev: CQEvent):
    ...
    replied_message = re.match(r".*?(\[CQ:reply,id=-?\d+])(\[CQ:at,qq=\d+]).*?",raw_message)
    ated_user_ids = set(re.findall(fr"\[CQ:at,qq=(\d+)]", raw_message))
    ...
    if replied_message:
        raw_message = raw_message.replace(replied_message.group(0),replied_message.group(1))

    for user_id in ated_user_ids:
        user_id = str(user_id)
        user_name = await get_member_name(bot=bot, gid=gid, uid=user_id)
        raw_message = raw_message.replace(f"[CQ:at,qq={user_id}]", f"@{user_name}").strip()

    for user_id in ated_user_ids:
        msg = node_custom(user_id=ev.user_id, name=sender_user_name, content=raw_message.strip(), time=get_now_time())
        if user_id in db:
